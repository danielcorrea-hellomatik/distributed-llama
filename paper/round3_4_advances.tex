...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\color{primary}Pushing the Pi5 cluster --- empirical ceiling investigation}
\fancyhead[R]{\small\thepage}
\fancyfoot[C]{\small\color{gray}Technical Note --- May 2026}
\renewcommand{\headrulewidth}{0.4pt}
\setlength{\headheight}{14pt}

\title{
  \vspace{-1cm}
  {\Huge\bfseries\color{primary} Pushing the Pi5 Cluster}\\[0.3cm]
  {\Large An empirical investigation of the bit-exact ceiling}\\[0.4cm]
  \rule{\textwidth}{1.5pt}\\[0.3cm]
  {\large Findings from 4 background Linux research agents and $\sim$20 cluster experiments}\\[0.3cm]
  {\normalsize Companion to ``Distributed LLM Inference on a 4-Node Raspberry Pi 5 Cluster''}
}
\author{
...
\begin{table}[H]
\centering
\caption{Measured throughput across the optimisation journey.}
\small
\begin{tabular}{lrrl}
\toprule
\textbf{Stage} & \textbf{Mean (tok/s)} & \textbf{$\pm$95\% CI} & \textbf{Cumulative} \\
\midrule
b4rtaz \#255 public ceiling (Pi5 8GB) & 13.04 & --- & baseline \\
Stage 1--8 baseline (\cite{originalreport}) & 13.720 & 0.050 & +5.2\% \\
\midrule
Stage 9 --- TIER 0 sysctls (GRO off, buffers, swap) & 14.011 & 0.051 & +7.45\% \\
A2 --- OP\_SILU\_MUL fusion (bit-exact) & 14.081 & 0.055 & +7.99\% \\
\midrule
Round 3a --- additional sysctls (autogroup, autocorking, etc) & 14.108 & 0.056 & noise \\
Round 3b --- madvise weights (RANDOM/DONTFORK) & 14.027 & 0.044 & noise \\
Round 3c --- per-thread CPU affinity 1-to-1 & 14.023 & 0.035 & noise \\
Round 3d --- threaded NAPI + deferred IRQ & \emph{12.904} & \emph{---} & \emph{regression $-3.9$\%} \\
\midrule
Round 4a --- EEVDF \texttt{sched\_setattr} slice 4ms & 14.061 & 0.048 & noise \\
Round 4b --- \texttt{vm.dirty\_bytes} cap 16MB & 14.027 & 0.044 & noise \\
Round 4c --- TCP buffers 8MB $\to$ 32MB & 13.968 & 0.038 & noise \\
Round 4d --- Phase B foundation + \texttt{--tile-sync 4} & \emph{12.904} & \emph{---} & \emph{regression $-8\%$} \\
Round 4e --- \texttt{max-seq-len} 32768 $\to$ 4096 & 13.965 & 0.027 & noise \\
\midrule
\textbf{Final (cold cluster, all bit-exact changes applied)} & \textbf{14.046} & \textbf{0.049} & \textbf{+2.38\%} vs baseline \\
& & & \textbf{+7.72\%} vs \#255 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Conclusion}: with the constraints of bit-exact computation, no kernel rebuild, and no quality compromise, the practical ceiling for this hardware running Qwen3-30B-A3B Q40 is $\sim$14.0 tok/s. We achieve 14.046 tok/s reproducibly when the cluster is cold-started.
...
We documented a multi-round empirical investigation pushing a 4-node Raspberry Pi 5 cluster running Qwen3-30B-A3B Q40 from its public ceiling of 13.04 tok/s up to 14.046 tok/s sustained, a \textbf{+7.72\% improvement} over the state-of-the-art for this exact hardware. The journey involved:
