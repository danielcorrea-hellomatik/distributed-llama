...
\noindent\includegraphics[height=0.95cm]{figures/hellomatik-logo.pdf}\par
\vspace{0.05cm}
...
{\normalsize\scshape Hellomatik.com \quad\textbullet\quad Laboratorio de Clúster Raspberry Pi}\\[0.26cm]
{\small\texttt{daniel.correa@hellomatik.com}}\\[0.45cm]
{\small DOI~\href{https://doi.org/10.5281/zenodo.20411979}{10.5281/zenodo.20411979} \quad\textbullet\quad 27 de mayo de 2026\\[0.10cm]
Complementario de \emph{Pushing Four Raspberry Pis to the Memory Wall} (Qwen3-30B-A3B, DOI~\href{https://doi.org/10.5281/zenodo.20357376}{10.5281/zenodo.20357376})}
\end{center}
\vspace{0.85cm}
\par}
...
\begin{itemize}
  \item \emph{Hardware y firmware}: el SoC, el subsistema de memoria y su techo de ancho de banda, el interconnect y los ajustes EEPROM (aquí, el intercalado de bancos bajos de SDRAM) --- el techo físico.
  \item \emph{Sistema operativo}: parámetros del kernel (governor, tamaño de página, sysctls de red, scheduler) y runtime (número de hilos, asignador, afinidad) --- cuánto del techo alcanza la carga.
  \item \emph{Software de aplicación}: el framework, sus kernels de cómputo, flags del compilador y algoritmos (fusión de operadores, cuantización, decodificación especulativa).
\end{itemize}

La regla que organiza todos los resultados de este informe --- una reformulación de la visión roofline, no una idea nueva --- es: \emph{perfila el cuello de botella y optimiza la capa donde vive.} En el decode denso el cuello de botella es el ancho de banda de memoria, así que las ganancias viven en el hardware y el sistema operativo: el cuarto hilo mueve el decode un 16\% por sí solo, y el intercalado de firmware y un kernel más nuevo suben el suelo al 29\% sobre el récord previo, mientras que reescribir los kernels de aplicación no produce diferencia medible (Welch $p{=}0.59$, Sección~\ref{sec:attribution}). Los mismos kernels sí rinden en una carga compute-bound --- 86\% en prefill, 15.2\% en decode MoE (Tabla~\ref{tab:layermap}) --- y esperamos que el método, no la receta, se traslade a otro software de inferencia (serving en GPU, modelos de detección); validarlo es trabajo futuro.
