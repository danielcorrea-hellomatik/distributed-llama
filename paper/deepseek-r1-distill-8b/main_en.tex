...
{\normalsize\scshape Hellomatik \quad\textbullet\quad Raspberry Pi Cluster Lab}\\[0.26cm]
...
Attribution vs.\ clean upstream (clean-room A/B) & fork ties stock at the four-thread optimum; contribution $\approx$0 & 8.08 / 8.09 \\
...
\label{sec:layers}
The results above are best read as a layered account of where inference performance comes from. We distinguish three layers:
\begin{itemize}
  \item \emph{Hardware and firmware}: the SoC, the memory subsystem and its bandwidth ceiling, the interconnect, and bootloader/EEPROM settings (here, SDRAM bank-low interleaving). This sets the physical ceiling.
  \item \emph{Operating system}: the kernel and its parameters (governor, page size, network sysctls, interrupt steering, scheduler) and the runtime configuration (thread count, allocator, affinity). This decides how much of the ceiling the workload actually reaches.
  \item \emph{Application software}: the inference framework, its compute kernels, compiler flags, and algorithms (operator fusion, quantisation, speculative decoding).
\end{itemize}

The single rule that organises every result in this report is: \emph{profile to locate the binding constraint, then spend optimisation effort in the layer where it lives.} For dense decode the constraint is memory bandwidth, so the gains are in the hardware and operating-system layers --- the fourth thread alone moves decode by 16\%, and the firmware interleave and a newer kernel raise the floor further, for 29\% over the prior record on the same model and framework. Rewriting the application kernels, by contrast, moves it by approximately zero (Section~\ref{sec:attribution}). The same kernels, applied to a compute-bound workload, do pay: 86\% on prefill and 15.2\% on the Mixture-of-Experts decode (Table~\ref{tab:layermap}).
...
\paragraph{Generalisation (our position).} The roofline and layer lens is workload-general, and we expect the method --- profile the binding constraint, then optimise the layer in which it lives --- rather than the prescription to carry to other inference software. Validating it on other stacks, for example GPU model serving or object-detection models, is future work.
