...
The cleanly isolated quantity --- vanilla v0.16.0 vs.\ our build on the \emph{same} 16~GB silicon --- is the controlled A/B we identify as the next measurement (Section~\ref{sec:threats}); until it is run, the same-hardware deltas we can defend without confound are the telemetry A/B above ($+5.18\%$) and the previously reported \texttt{nthreads} and OS-tuning A/Bs. Raw per-run CSVs for both arms are committed alongside this manuscript (\texttt{paper/data/}).
