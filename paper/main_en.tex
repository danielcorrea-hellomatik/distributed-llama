...
\title{
  ...
  {\normalsize Qwen3-30B-A3B (MoE) over patched distributed-llama: \textbf{14.449 tok/s sustained} (+10.81\% above the publicly documented ceiling of 13.04 tok/s; best individual run 14.557; trajectory 12.71 $\rightarrow$ 14.011 $\rightarrow$ 14.046 $\rightarrow$ 14.449 through Stages 6, 9, 11, 12--15)}
}
\author{
...
\section{Post-publication update --- Stage 9 (TIER 0 OS-level tuning)}
\label{sec:stage9}
...
\section{Post-publication update --- Stages 10 and 11 (A2 op-fusion + Rounds 3 and 4)}
...
\subsection{Updated bottom line (post Stage 11)}

After Stage 11 the cluster sustained \textbf{14.046 tok/s} ($\pm$0.049 at 95\% CI, $n$=20, cold-cluster), \textbf{+7.72\% above the publicly documented ceiling} of 13.04 tok/s. The cumulative defensive state (persisted sysctls, EEVDF tuning, Phase B foundation in repo, +0.5\% A2 fusion) represented a maintainable production baseline at the time of the post-publication update.

\subsection{Post-Stage-11 session (2026-05-18): Stages 12--15}
...
\subsection{Final bottom line}

The cluster sustains \textbf{14.449 tok/s} ($\pm$0.038 at 95\% CI, $n$=20, cold-cluster; best individual run 14.557), \textbf{+10.81\% above the publicly documented ceiling} of 13.04 tok/s for Qwen3-30B-A3B Q40 on $4\times$Pi 5. The four Stage 12--15 commits, combined with the post-publication updates of Stages 9--11, represent a $+5.31\%$ improvement over the original Stage 8 baseline of 13.720 tok/s and a maintainable, fully bit-exact production state. To our knowledge no publicly documented configuration on equivalent hardware exceeds this throughput at the time of writing.

\subsection{Future work}

The only software lever above the noise floor under bit-exact constraints is \textbf{Phase B Option C} (estimated $+5$--$12\%$):

\begin{itemize}[leftmargin=*,nosep]
  \item Add \texttt{NnAsyncSyncContext *asyncSync} field to \texttt{NnCpuOpContext}.
  \item Modify \texttt{matmulForward\_Q80\_Q40\_F32} to dispatch to the tiled variant when \texttt{asyncSync} is set, starting the drain thread before kernel completion.
  \item Modify \texttt{NnNetworkNodeSynchronizer::sync} to call \texttt{nnAsyncSyncJoin()} on the drain thread's context.
  \item Op builder in \texttt{llm.cpp} attaches \texttt{NnAsyncSyncContext} to specific Q80$\times$Q40 matmul ops whose output is the next sync's source pipe.
  \item 50 ms watchdog timeout in the drain thread, \texttt{static\_assert(d \% K == 0)} runtime check, bit-exact 100-token validation at \texttt{seed=42}, and a 60-minute soak test.
\end{itemize}
...
A separate companion paper, \texttt{paper/round3\_4\_advances.tex}, contains a more detailed academic-style write-up of this investigation including the full table of negative findings and the four-agent research methodology.
