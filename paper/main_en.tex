...
\title{Hitting the Memory Wall on Four Raspberry Pis:\\[0.15cm]
       Bit-Exact 30B Mixture-of-Experts Inference at the Edge,\\
       +16\% over the Public Record}
\hypersetup{pdftitle={Hitting the Memory Wall on Four Raspberry Pis: Bit-Exact 30B Mixture-of-Experts Inference at the Edge},pdfauthor={Daniel Correa Villa}}
...
\vspace*{3.5cm}
\begin{center}
  {\LARGE Hitting the Memory Wall on Four Raspberry Pis\par}
  \vspace{0.4cm}
  {\large Bit-Exact 30B Mixture-of-Experts Inference at the Edge,\\[0.15cm]
...
Reaching this ceiling required adapting the host operating system as heavily as the application itself. Across the four nodes we: pinned the CPU governor to \texttt{performance} at the nominal 2.4~GHz; set \texttt{SDRAM\_BANKLOW=1} in the bootloader EEPROM --- a firmware-level memory-interleave change that lifted effective read bandwidth from 8.3 to 12.5~GB/s per node, bit-exact, for $+5.9\%$; ran the inference with \texttt{--nthreads 3} rather than 4, freeing one core, and pinned the Ethernet IRQ and Receive Packet Steering to that freed core (CPU~3); preloaded \texttt{jemalloc} with a tuned arena and tcache configuration; \texttt{mlock}-ed the model weights into RAM (\texttt{LimitMEMLOCK=infinity}, $\sim$4.45~GB locked per worker); switched the network stack to BBR with enlarged \texttt{SO\_RCVBUF}/\texttt{SO\_SNDBUF}, disabled GRO, and enabled \texttt{busy\_poll}; lowered \texttt{vm.swappiness} to 1 and placed swap on NVMe; set the NVMe scheduler to \texttt{none} with a larger read-ahead; and enlarged the NIC RX ring with Receive Flow Steering and deferred NAPI. The full state is captured in four \texttt{systemd} units (\texttt{dllama-api}, \texttt{dllama-worker}, \texttt{cpu-performance}, \texttt{dllama-irqpin}) plus a runtime-tweaks unit, so a cold cluster boots straight into the optimised configuration. We also found that the operating environment must be considered in full: the clean-room experiment of Section~\ref{sec:cleanroom-2026-05-22} showed that even a co-located observability stack (Grafana Alloy and cAdvisor) silently taxes decode throughput by $\sim$5\%. Every one of these levers was held to the bit-exact constraint; several plausible candidates (transparent huge pages, jumbo frames, NUMA interleave, software prefetch, PGO) were tested and \emph{reverted} as neutral or harmful on this DRAM-bandwidth-bound workload.

Taken together, these results drive a four-node Raspberry Pi~5 cluster to the physical ceiling of its LPDDR4X memory subsystem. At 49\% backend-stalled cycles and roughly 11.4 of the $\sim$17~GB/s vendor bandwidth realised per node, decode is memory-bandwidth-bound: the bytes moved per token are fixed by the model and its quantisation, and no further bit-exact \emph{reduction} of that traffic is available --- the one lever left (Async-TP) overlaps communication with computation rather than moving fewer bytes. The telemetry experiment of Section~\ref{sec:cleanroom-2026-05-22} sharpens the point: even the few percent of DRAM bandwidth consumed by a background monitoring agent is directly visible in the decode rate, while the compute-bound prefill phase is untouched. We have, in short, reached the \emph{memory wall} that the Raspberry~Pi~5 presents. To our knowledge, \textbf{15.143~tok/s} is the highest rate at which a 30-billion-parameter Mixture-of-Experts model has been driven on this hardware class --- bit-exact, with no overclock and no loss of output quality. Pushing past it is no longer a software problem but a silicon one: it requires memory with more bandwidth.
