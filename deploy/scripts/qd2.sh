#!/usr/bin/env bash
# Parametrized quick decode probe. Env: BIN NT JEM STEPS N
set -uo pipefail
BIN="${BIN:?}"; NT="${NT:-3}"; JEM="${JEM:-1}"; STEPS="${STEPS:-160}"; N="${N:-3}"
...
for r in $(seq 1 "$N"); do
  out=$("$BIN" inference --model "$MODEL" --tokenizer "$TOK" --buffer-float-type q80 --max-seq-len 4096 \
    ...
    --steps "$STEPS" --nthreads "$NT" --workers $WORKERS 2>/dev/null)
